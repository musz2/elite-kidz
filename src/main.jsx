...
import { formatRupees, priceFor, SALE_PERCENT } from "./data/prices";
import "./styles.css";
import "./premium.css";
...
// Sale price beside the struck-through original. Pieces without a confirmed
// price keep the enquiry line.
function Price({ product, className = "", as: Tag = "p" }) {
  const price = priceFor(product);
  if (!price) return <Tag className={`price ${className}`}>Price on enquiry</Tag>;
  return <Tag className={`price ${className}`}>
    <span className="price-sale"><span className="visually-hidden">Sale price </span>{formatRupees(price.sale)}</span>
    <s className="price-original"><span className="visually-hidden">Original price </span>{formatRupees(price.original)}</s>
    <span className="price-off">{SALE_PERCENT}% OFF</span>
  </Tag>;
}
...
function Header({ cartCount, onOpenCart, onOpenSearch }) {
  ...
  // The open menu starts where the header ends, which moves with the bars above
  // it and with how far the page is scrolled.
  ...
  return <>
    ...
    <header ref={header} className="site-header" onKeyDown={menuOpen ? trapDialogFocus : undefined}>
      ...
      <nav className={`main-nav ${menuOpen ? "is-open" : ""}`} style={menuOpen && navTop ? { top: navTop } : undefined} aria-label="Primary navigation">
...
function ProductCard({ product, onQuickAsk, onAdd }) {
  ...
  return <article className="product-card">
    ...
    <div className="product-meta"><div><p className="product-collection">{product.type} <span>{product.itemNumber}</span></p><h3><a href={`#/product/${product.slug}`}>{product.name}</a></h3><p className="product-colour">{product.colour}</p><Price product={product} className="price-card" /></div><button className="mini-whatsapp" aria-label={`Ask about ${product.name} on WhatsApp`} onClick={() => onQuickAsk(product)}><Icon name="whatsapp" size={18} /></button></div>
  </article>;
}
...
function ProductPage({ product, onAdd, onQuickAsk }) {
  ...
  return <main id="main-content" className="product-page section-shell"><nav className="breadcrumbs" aria-label="Breadcrumb"><a href="#/shop">The collection</a><span>/</span><a href={`#/shop?type=${product.type}`}>{product.type}</a><span>/</span><span>{product.name}</span></nav><div className="product-layout"><ProductGallery product={product}/><div className="product-info"><p className="eyebrow">{product.category} · {product.type} · {product.itemNumber}</p><h1>{product.name}</h1><Price product={product} className="pdp-price" />{product.description&&<p className="product-lede">{product.description}</p>}<div className="product-rule"/><p className="product-colour-label"><span>Colour</span> {product.colour}</p><div className="size-block"><div className="option-label"><span>Select a size</span><a href={`#/size-guide?style=${product.itemNumber}`} onClick={event=>handleInternalNavigation(event,`size-guide?style=${product.itemNumber}`)}>Size guide ↗</a></div><div className="size-options" role="group" aria-label={`Choose a size for ${product.name}`}>{product.sizes.map(size=><button key={size} className={selectedSize===size?'active':''} aria-pressed={selectedSize===size} aria-label={`Size ${size}, for a height of ${sizeHeight(size)}`} onClick={()=>setSelectedSize(size)}><strong>{size}</strong><small>{sizeHeight(size)}</small></button>)}</div><p className="stock-note">Sizes are cut to the child&rsquo;s height &mdash; {product.sizes.length ? `${product.sizes[0]} fits ${sizeHeight(product.sizes[0])}` : "see the size guide"}. Availability is confirmed on WhatsApp.</p></div><div className="quantity-block"><span className="option-label">Quantity</span><div className="quantity-control"><button disabled={quantity===1} aria-label="Decrease quantity" onClick={()=>setQuantity(q=>Math.max(1,q-1))}><Icon name="minus" size={16}/></button><span aria-live="polite">{quantity}</span><button aria-label="Increase quantity" onClick={()=>setQuantity(q=>q+1)}><Icon name="plus" size={16}/></button></div></div><div className="product-actions"><button className="button button-dark button-wide" disabled={!selectedSize} onClick={()=>onAdd(product,quantity,selectedSize)}><Icon name="bag" size={18}/>{selectedSize?'Add to bag':'Select a size to add'}</button><button className="button button-outline button-wide" onClick={()=>onQuickAsk(product,selectedSize,quantity)}><Icon name="whatsapp" size={18}/> Ask about this piece</button></div><p className="action-note">Your bag is an enquiry. We’ll confirm price, availability and delivery before you order.</p><div className="pdp-details">{product.detail&&<details><summary>About this piece <Icon name="plus" size={16}/></summary><p>Style {product.itemNumber}. {product.description || `${product.type} in ${product.colour.toLowerCase()}.`}</p></details>}{product.material&&<details><summary>Material <Icon name="plus" size={16}/></summary><p>{product.material}</p></details>}<details><summary>Ordering & delivery <Icon name="plus" size={16}/></summary><p>Choose your size and add your favourites to the bag. Continue on WhatsApp to confirm prices, stock and delivery with Elite Kidz.</p><a href="#/shipping-returns">Ordering information ↗</a></details></div></div></div><section className="related-products"><SectionHeading title="A few more little favourites" action="Explore the collection" onAction={()=>navigate('shop')}/><div className="product-grid">{products.filter(p=>p.id!==product.id).slice(0,4).map(p=><ProductCard key={p.id} product={p} onAdd={onAdd} onQuickAsk={onQuickAsk}/>)}</div></section></main>;
}

// Shown for a product link that no longer resolves — mistyped, shared from an
// old catalogue, or a piece that has been removed — and for any unknown page.
function NotFound({ kind = "page" }) {
  const isProduct = kind === "product";
  const suggestions = products.filter((p) => p.featured).slice(0, 4);
  return <main id="main-content" className="content-page not-found-page section-shell">
    <div className="not-found-intro">
      <p className="eyebrow">{isProduct ? "Product not found" : "Page not found"}</p>
      <h1>{isProduct ? <>This little piece<br /><em>has wandered off.</em></> : <>This page<br /><em>has wandered off.</em></>}</h1>
      <p>{isProduct ? "The link may be mistyped, or this piece is no longer in the collection. The rest of the wardrobe is just a tap away." : "The link may be mistyped or out of date. Let’s get you back to the collection."}</p>
      <div className="not-found-actions">
        <a className="button button-dark" href="#/shop" onClick={(event) => handleInternalNavigation(event, "shop")}>Back to shop <Icon name="arrow" size={18} /></a>
        <a className="button button-outline" href="#/" onClick={(event) => handleInternalNavigation(event, "")}>Go home</a>
      </div>
      <p className="not-found-help">Looking for something specific? <a href={`https://wa.me/${WHATSAPP_NUMBER}`} target="_blank" rel="noreferrer">Ask Elite Kidz on WhatsApp</a></p>
    </div>
    <section className="not-found-suggestions" aria-labelledby="not-found-suggestions-title">
      <p className="eyebrow" id="not-found-suggestions-title">Still in the collection</p>
      <div className="not-found-grid">{suggestions.map((p) => <a key={p.id} href={`#/product/${p.slug}`} onClick={(event) => handleInternalNavigation(event, `product/${p.slug}`)}>
        <span className="not-found-photo"><img src={imagePreview(cardImage(p))} alt={p.alt} loading="lazy" /></span>
        <strong>{p.name}</strong>
        <Price product={p} as="span" />
      </a>)}</div>
    </section>
  </main>;
}
...
function App() {
  ...
  useEffect(() => { document.title = route.page === "product" ? (getProduct(route.slug) ? `${getProduct(route.slug).name} — Elite Kidz` : "Product not found — Elite Kidz") : route.page === "shop" ? "Shop — Elite Kidz" : route.page === "faq" ? "FAQ — Elite Kidz" : route.page === "size-guide" ? "Size Guide — Elite Kidz" : route.page === "shipping-returns" ? "Shipping & Returns — Elite Kidz" : "Elite Kidz — Made for Little Stars"; }, [route]);
