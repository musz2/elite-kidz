# Elite Kidz storefront

Real kidswear photography is the visual foundation. Preserve the existing Playfair Display / DM Sans identity; use rose ink (#803747), deep warm text (#352b2c), sage accents and quiet light surfaces. Real merchandise and clear ordering take priority over motion.

The homepage pairs two supplied photographs in a restrained paper-like composition. A small pointer depth offset applies only on wide, fine-pointer screens. Featured products come from the catalog flag; product types, colours, sizes, search and counts derive from the same inventory. No fabricated campaigns, prices, categories, testimonials or subscription forms.
